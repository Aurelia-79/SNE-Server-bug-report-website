...
from server.services.domain import create_user_with_profile, utcnow


def init_database() -> None:
    settings.upload_dir.mkdir(parents=True, exist_ok=True)
    Base.metadata.create_all(bind=engine)
    _migrate_users_email()
    _migrate_users_avatar_path()
    _migrate_staff_profiles_department_nullable()


def _migrate_users_email() -> None:
...
def _migrate_users_avatar_path() -> None:
    inspector = inspect(engine)
    if "users" not in inspector.get_table_names():
        return
    columns = {column["name"] for column in inspector.get_columns("users")}
    if "avatar_path" in columns:
        return
    with engine.begin() as connection:
        connection.execute(text("ALTER TABLE users ADD COLUMN avatar_path VARCHAR(255) NULL"))
