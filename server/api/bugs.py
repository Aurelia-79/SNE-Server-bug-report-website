...
from server.models.entities import BugAttachment, BugComment, BugStatus, BugTicket, SystemRole, User
from server.schemas.api import BugCommentCreateRequest, BugTicketCreateRequest, BugTicketUpdateRequest
from server.services.domain import (
    collect_admin_emails,
    build_ticket_comment_mail,
    build_ticket_created_mail,
    build_ticket_reply_mail,
    create_audit_log,
    ensure_bug_management_permission,
    get_staff_or_404,
...
def create_ticket(
    ...
    if admin_emails:
        subject, text_body, html_body = build_ticket_created_mail(
            ticket_id=ticket.id,
            title=ticket.title,
        ...
        send_notification_email(db, to_addresses=admin_emails, subject=subject, body=text_body, html=html_body)
...
def add_comment(
    ...
    db.commit()
    # Notify reporter when someone else comments
    if ticket.reporter_id != current_user.id:
        reporter = get_staff_or_404(db, ticket.reporter_id)
        if reporter.email:
            subject, text_body, html_body = build_ticket_comment_mail(
                ticket_id=ticket.id,
                title=ticket.title,
                commenter_name=current_user.display_name,
                comment_text=payload.content,
            )
            send_notification_email(db, to_addresses=[reporter.email], subject=subject, body=text_body, html=html_body)
    return build_bug_payload(db, ticket)
...
def update_ticket(
    ...
    db.commit()
    if data.get("status") and data.get("comment") and data["status"] != previous_status:
        reporter = get_staff_or_404(db, ticket.reporter_id)
        if reporter.email:
            subject, text_body, html_body = build_ticket_reply_mail(
                ticket_id=ticket.id,
                title=ticket.title,
